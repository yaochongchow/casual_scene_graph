...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
                 domain_features, critic_model: Optional[nn.Module] = None):
        super().__init__()
        ...
        self.critic_model = critic_model  # IRM critic model for edge scoring
        if self.critic_model is not None:
            self.critic_model.eval()  # Set to eval mode
    ...
    def sample_p_zs_given_zt_ddim(self, s, t, X_t, E_t, y_t, node_mask, beta_t, guidance_scale=1.0):
        ...
        # Classifier-Free Guidance with Critic Edge Scores
        if guidance_scale != 1.0:
            ...
            # Get edge scores from critic if available
            edge_scores_list = None
            if self.critic_model is not None:
                edge_scores_list = self._get_edge_scores_from_critic(X_t, E_t, node_mask)
            
            # Mix logits with critic-based guidance
            ...
            # Apply critic-based edge guidance if available
            if edge_scores_list is not None:
                # For each graph in batch, weight edge predictions by critic scores
                for i, edge_scores in enumerate(edge_scores_list):
                    # Convert edge_scores to dense adjacency weights
                    # We need to map edge_index back to (n, n) adjacency
                    # For now, we'll apply a simple weighting scheme
                    # This is a simplified version - full implementation would need proper edge mapping
                    # TODO: Implement proper edge score weighting for dense E tensor
    ...
    def _dense_to_pyg_data(self, X_t, E_t, node_mask, batch_idx=0):
        """Convert dense tensors to PyG Data for critic evaluation.
        Args:
            X_t: (bs, n, dx) node features (one-hot or logits)
            E_t: (bs, n, n, de) edge features (one-hot or logits)
            node_mask: (bs, n) boolean mask
            batch_idx: which graph in batch to convert
        Returns:
            Data object for single graph
        """
        # Extract single graph
        n = node_mask[batch_idx].sum().item()
        x = X_t[batch_idx, :n]  # (n, dx)
        # Convert to node features (if one-hot, take argmax; if logits, take softmax then argmax)
        if x.dim() > 1 and x.size(-1) > 1:
            x = torch.argmax(x, dim=-1).unsqueeze(-1).float()  # (n, 1)
        else:
            x = x.float()
        
        # Extract edge adjacency
        E_single = E_t[batch_idx, :n, :n]  # (n, n, de)
        # Convert to adjacency matrix
        if E_single.dim() == 3 and E_single.size(-1) > 1:
            # One-hot or logits: take argmax
            E_single = torch.argmax(E_single, dim=-1)  # (n, n)
        else:
            E_single = E_single.squeeze(-1)  # (n, n)
        
        # Convert to edge_index
        edge_index, edge_attr = dense_to_sparse(E_single)
        
        return Data(x=x, edge_index=edge_index, num_nodes=n)
    
    def _get_edge_scores_from_critic(self, X_t, E_t, node_mask):
        """Get edge scores from critic for all graphs in batch.
        Returns:
            List of edge score tensors, one per graph
        """
        ...
        for i in range(bs):
            pyg_data = self._dense_to_pyg_data(X_t, E_t, node_mask, batch_idx=i)
            pyg_data = pyg_data.to(self.device)
            pyg_data.batch = torch.zeros(pyg_data.num_nodes, dtype=torch.long, device=self.device)
            
            with torch.no_grad():
                outputs = self.critic_model(pyg_data)
                edge_scores = outputs["edge_scores"]  # (num_edges,)
